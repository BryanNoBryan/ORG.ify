                    ...
                    FutureBuilder(
                      ...
                      builder: (context, snapshot2) {
                        if (snapshot2.hasData) {
                          bool signedUpAlready = snapshot2.data!
                              .where((e) =>
                                  e.eventId ==
                                      snapshot.data![position].eventId &&
                                  e.username == UserState.name)
                              .isNotEmpty;
                          ...
                          snapshot2.data!
                              .where((e) =>
                                  e.eventId ==
                                      snapshot.data![position].eventId &&
                                  e.username == UserState.name)
                              .forEach((e) => testS += e.username);

                          log(testS);
                          log(signedUpAlready.toString());

                          return GestureDetector(
                            onTap: () async {
                              if (!signedUpAlready) {
                                SignUp su = SignUp(
                                    eventId: snapshot.data![position].eventId!,
                                    username: UserState.name);
                                SignUpDB().insertElem(su);
                              } else {
                                SignUpDB().deleteElem(snapshot2.data!
                                    .firstWhere(
                                        (e) => e.username == UserState.name)
                                    .id!);
                              }
                              setState(() {});
                            },
